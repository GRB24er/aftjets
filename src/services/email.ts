...
export async function sendEmail(options: EmailOptions): Promise<boolean> {
  try {
    await transporter.sendMail({
      from: process.env.EMAIL_FROM || '"ATF Jets" <noreply@atfjets.com>',
      ...options,
    });
    return true;
  } catch (error) {
...
<div style="max-width:600px;margin:0 auto;background:#0a0f1e;border:1px solid rgba(255,255,255,0.06);">
  <div style="padding:32px;text-align:center;background:linear-gradient(135deg,rgba(99,102,241,0.15),rgba(34,211,238,0.08));">
    <div style="font-size:24px;font-weight:bold;color:#f0f0f5;letter-spacing:2px;">ATF <span style="color:#c9a96e;font-size:12px;letter-spacing:3px;">JETS</span></div>
  </div>
  ...
  <div style="padding:24px;text-align:center;border-top:1px solid rgba(255,255,255,0.06);">
    <p style="color:#5a6480;font-size:11px;margin:0;">&copy; ${new Date().getFullYear()} ATF Jets. All rights reserved.</p>
  </div>
</div></body></html>`;
}
...
<div style="max-width:600px;margin:0 auto;background:#0a0f1e;border:1px solid rgba(255,255,255,0.06);">
  <div style="padding:32px;background:linear-gradient(135deg,rgba(99,102,241,0.2),rgba(34,211,238,0.1));text-align:center;">
    <div style="font-size:24px;font-weight:bold;color:#f0f0f5;letter-spacing:2px;">ATF <span style="color:#c9a96e;font-size:12px;letter-spacing:3px;">JETS</span></div>
    <div style="color:#c9a96e;font-size:12px;letter-spacing:3px;margin-top:8px;">E-TICKET / BOARDING PASS</div>
  </div>
  <div style="padding:32px;">
  ...
  <div style="padding:16px;text-align:center;border-top:1px solid rgba(255,255,255,0.06);">
    ...
    <p style="color:#5a6480;font-size:10px;margin:4px 0 0;">&copy; ${new Date().getFullYear()} ATF Jets</p>
  </div>
</div></body></html>`;
}
