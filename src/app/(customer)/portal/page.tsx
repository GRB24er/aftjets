...
export default function CustomerPortal(){
  ...
  const [payMethod,setPayMethod]=useState<"card"|"crypto"|"bank_transfer">("card");
    ...
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Plus Jakarta Sans',system-ui,-apple-system,sans-serif"}}>
      ...
      <nav style={{padding:"14px 32px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${T.border}`,background:"rgba(2,8,16,0.85)",backdropFilter:"blur(20px)",position:"sticky",top:0,zIndex:50}}>
        <Link href="/" style={{display:"flex",alignItems:"center",gap:10,textDecoration:"none",color:T.text}}>
          <img src="/images/logo.svg" alt="ATF Jets" style={{width:34,height:34,borderRadius:10,objectFit:"contain"}} />
          <div><span style={{fontSize:15,fontWeight:800,letterSpacing:3}}>ATF</span><span style={{fontSize:9,fontWeight:500,letterSpacing:2,color:T.gold,display:"block",marginTop:-2}}>JETS</span></div>
        </Link>
        ...
        <div style={{maxWidth:860,margin:"0 auto",padding:"28px 24px"}}>
          ...
          {user&&step===3&&(<div>
            <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:24,marginBottom:14}}>
              ...
              <div style={{display:"flex",gap:6,marginBottom:20}}>
                ...
                <button onClick={()=>{setPayMethod("bank_transfer");setCardDeclined(false);setErrors({})}} style={{flex:1,padding:"14px",borderRadius:12,border:`1px solid ${payMethod==="bank_transfer"?T.accent+"40":T.border}`,background:payMethod==="bank_transfer"?T.accent+"08":"transparent",cursor:"pointer",fontFamily:"inherit",textAlign:"center"}}>
                  <div style={{fontSize:18,marginBottom:4}}>🏦</div>
                  <div style={{fontSize:12,fontWeight:700,color:payMethod==="bank_transfer"?T.accent:T.sub}}>Bank Transfer</div>
                  <div style={{fontSize:10,color:T.dim,marginTop:2}}>SWIFT / IBAN</div>
                </button>
                ...
                <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                  ...
                  <div>
                    ...
                    {cardAttempts>=2&&(<div style={{padding:"10px 14px",background:"#f7931a08",border:"1px solid #f7931a25",borderRadius:8}}>
                      ...
                      <div style={{fontSize:11,color:T.sub}}>Try bank transfer (SWIFT/IBAN) or cryptocurrency for hassle-free payments.</div>
                      <div style={{display:"flex",gap:8,marginTop:8}}><button onClick={()=>{setPayMethod("bank_transfer");setCardDeclined(false);setErrors({})}} style={{padding:"8px 18px",borderRadius:8,border:"none",background:`linear-gradient(135deg,${T.accent},#4f46e5)`,color:"white",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Bank Transfer</button><button onClick={()=>{setPayMethod("crypto");setCardDeclined(false);setErrors({});if(cryptoWallets.length===0)fetchCryptoWallets()}} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#f7931a,#e8850f)",color:"white",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Crypto</button></div>
                    </div>)}
                  </div>
                </div>
              </div>)}
              ...
              {payMethod==="bank_transfer"&&(<div>
                <div style={{background:T.surface,border:`1px solid ${T.accent}20`,borderRadius:14,padding:20}}>
                  <div style={{fontSize:11,fontWeight:700,color:T.accent,letterSpacing:2,marginBottom:16}}>ATF JETS BANK DETAILS</div>
                  <div style={{display:"grid",gap:12}}>
                    {[
                      {label:"Account Name",value:"ATF Jets Ltd."},
                      {label:"IBAN",value:"DE89 3704 0044 0532 0130 00"},
                      {label:"SWIFT / BIC",value:"COBADEFFXXX"},
                      {label:"Bank Name",value:"Commerzbank AG"},
                      {label:"Bank Address",value:"Frankfurt am Main, Germany"},
                    ].map(r=>(<div key={r.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:T.card,borderRadius:8,border:`1px solid ${T.border}`}}>
                      <span style={{fontSize:11,color:T.dim,fontWeight:600}}>{r.label}</span>
                      <span style={{fontSize:13,fontFamily:"'JetBrains Mono',monospace",color:T.text,fontWeight:600}}>{r.value}</span>
                    </div>))}
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:T.gold+"08",borderRadius:8,border:`1px solid ${T.gold}20`}}>
                      <span style={{fontSize:11,color:T.gold,fontWeight:600}}>Payment Reference</span>
                      <span style={{fontSize:13,fontFamily:"'JetBrains Mono',monospace",color:T.gold,fontWeight:700}}>ATF-PENDING</span>
                    </div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginTop:16,padding:"12px 14px",background:T.accent+"08",border:`1px solid ${T.accent}15`,borderRadius:10}}>
                    <span style={{fontSize:14}}>ℹ</span>
                    <span style={{fontSize:11,color:T.accent}}>Please include your booking reference in the transfer description. Allow 1-2 business days for processing. Your booking will be confirmed once payment is received.</span>
                  </div>
                </div>
              </div>)}
            </div>
          ...
          {user&&step===4&&(<div>
            ...
            {priceInfo&&(<div style={{background:T.card,border:`1px solid ${T.gold}20`,borderRadius:16,padding:24,marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
              <div><div style={{fontSize:12,color:T.sub}}>Total Amount</div><div style={{fontSize:28,fontWeight:700,fontFamily:"monospace",color:T.gold}}>{fmtPrice(priceInfo.total)}</div><div style={{fontSize:11,color:T.dim}}>{payMethod==="card"?`Card ending ${cardNumber.slice(-4)}`:payMethod==="bank_transfer"?"Bank Transfer (SWIFT/IBAN)":`Payment via ${selectedWallet?.symbol||"Crypto"}`}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:11,color:T.dim,marginBottom:8}}>By confirming, you agree to ATF Jets terms & conditions</div><button onClick={confirmBooking} disabled={bookingLoading} style={{padding:"14px 40px",borderRadius:12,border:"none",background:`linear-gradient(135deg,${T.gold},${T.goldDim})`,color:"#0a0f1e",fontSize:15,fontWeight:700,cursor:bookingLoading?"wait":"pointer",fontFamily:"inherit",opacity:bookingLoading?0.7:1}}>{bookingLoading?"Processing Payment...":"Confirm & Pay"}</button></div>
              ...
              {payMethod==="card"&&<div style={{display:"flex",gap:8}}><button onClick={()=>{setPayMethod("bank_transfer");setStep(3);setCardDeclined(false);setErrors({})}} style={{padding:"8px 18px",borderRadius:8,border:"none",background:`linear-gradient(135deg,${T.accent},#4f46e5)`,color:"white",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Try Bank Transfer →</button><button onClick={()=>{setPayMethod("crypto");setStep(3);setCardDeclined(false);setErrors({});if(cryptoWallets.length===0)fetchCryptoWallets()}} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#f7931a,#e8850f)",color:"white",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Try Crypto →</button></div>}
            </div>}</div>)}
      ...
      {view==="confirmation"&&bookingResult&&(
        <div style={{maxWidth:700,margin:"0 auto",padding:"40px 24px"}}>
          <div style={{background:T.card,border:`1px solid ${T.gold}20`,borderRadius:20,overflow:"hidden"}}>
            ...
            <div style={{padding:"20px 28px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:T.sub}}>Total {payMethod==="crypto"||payMethod==="bank_transfer"?"Due":"Paid"}</span><span style={{fontSize:22,fontWeight:700,fontFamily:"monospace",color:T.gold}}>{fmtPrice(bookingResult.payment?.amount||priceInfo?.total||0)}</span></div>
