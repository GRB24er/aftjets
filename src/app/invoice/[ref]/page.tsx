...
export default function InvoicePage({ params }: { params: { ref: string } }) {
  ...
  const generatePDF = async () => {
    ...
    doc.text("ATF Jets Ltd. | admin@atfjets.com | This is a computer-generated invoice.", W / 2, 280, { align: "center" });
    doc.save("ATF_Invoice_" + data.bookingReference + ".pdf");
  };
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:"40px 16px",fontFamily:"'Segoe UI',Arial,sans-serif"}}>
      ...
      <div style={{maxWidth:700,margin:"0 auto",animation:"fadeUp 0.6s ease"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
          <div style={{fontSize:20,fontWeight:700,letterSpacing:2,color:C.text}}>ATF <span style={{color:C.accent,fontSize:10,letterSpacing:3}}>JETS</span></div>
        ...
        <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,overflow:"hidden"}}>
          ...
          <div style={{padding:"16px 28px",borderTop:"1px solid "+C.border,display:"flex",justifyContent:"space-between"}}>
            ...
            <span style={{fontSize:10,color:C.dim}}>ATF Jets Ltd.</span>
          </div>
        </div>
      </div>
    </div>
