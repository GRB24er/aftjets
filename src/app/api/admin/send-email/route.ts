  ...
  try {
    ...
    if (type === "confirmation") {
      sent = await sendEmail({
        to: email,
        subject: `ATF Jets - Booking Confirmed ${booking.bookingReference}`,
      ...
      sent = await sendEmail({
        to: email,
        subject: `ATF Jets - E-Ticket ${booking.bookingReference}`,
        html: eTicketEmail({
          name: `${pax?.firstName || ""} ${pax?.lastName || ""}`,
          bookingRef: booking.bookingReference,
          flightNumber: flight?.flightNumber || "N/A",
...
<div style="max-width:600px;margin:0 auto;background:#0a0f1e;border:1px solid rgba(255,255,255,0.06);">
  <div style="padding:32px;text-align:center;background:linear-gradient(135deg,rgba(99,102,241,0.15),rgba(34,211,238,0.08));">
    <div style="font-size:24px;font-weight:bold;color:#f0f0f5;letter-spacing:2px;">ATF <span style="color:#818cf8;font-size:12px;letter-spacing:3px;">JETS</span></div>
  </div>
  ...
  <div style="padding:24px;text-align:center;border-top:1px solid rgba(255,255,255,0.06);">
    <p style="color:#5a6480;font-size:11px;margin:0;">&copy; ${new Date().getFullYear()} ATF Jets</p>
  </div>
</div></body></html>`;
}
