...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!aircraft) {
      aircraft = await Aircraft.create({ name: "Boeing 787-9 Dreamliner", model: "787-9", type: "wide-body", manufacturer: "Boeing", registration: "ATF-GEN-002", totalSeats: 290, status: "active" });
    }
    ...
    const flight = await Flight.create({
      flightNumber: flightNumber.toUpperCase(), type: "commercial", airline: "ATF Jets", aircraft: aircraft._id,
