...
const _f = Flight;
const BASE = process.env.NEXT_PUBLIC_APP_URL || "https://atfjets.com";
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const booking of bookings) {
      ...
      try { await sendEmail({ to: booking.contactEmail, subject: "ATF Jets - Flight " + flight.flightNumber + " " + status.charAt(0).toUpperCase() + status.slice(1), html }); emailsSent++; } catch (e) { console.error("Email err:", e); }
    }
...
function buildStatusEmail(name: string, statusMsg: string, statusColor: string, flight: any, bookingRef: string): string {
  return '<!DOCTYPE html><html><head><meta charset="utf-8"></head>' +
    ...
    '<div style="font-size:24px;font-weight:bold;color:#f0f0f5;letter-spacing:2px;">ATF <span style="color:#818cf8;font-size:12px;letter-spacing:3px;">JETS</span></div></div>' +
    ...
    '</div>' +
    '<div style="padding:16px;text-align:center;border-top:1px solid rgba(255,255,255,0.06);"><p style="color:#5a6480;font-size:10px;margin:0;">&copy; ' + new Date().getFullYear() + ' ATF Jets</p></div>' +
    '</div></body></html>';
}
