 ...
 * 1. Flight number format: "ATF 123" (matches search route)
...
function generateFlightsForBooking(fc:string,tc:string,ds:string,aircraftId:any){
  ...
  for(let i=0;i<n;i++){
    ...
    const fn=`ATF ${String(100+Math.round(r()*899))}`;
    out.push({
      flightNumber:fn,type:"commercial" as const,airline:"ATF Jets",isActive:true,
      aircraft:aircraftId,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (generatedIds.length > 0) {
      ...
      let defaultAircraft = await Aircraft.findOne({ registration: "ATF-GEN-001" });
      if (!defaultAircraft) {
        defaultAircraft = await Aircraft.create({
          registration: "ATF-GEN-001", name: "Boeing 787-9 Dreamliner", manufacturer: "Boeing", model: "787-9",
    ...
    try {
      await sendEmail({
        to: contactEmail,
        subject: `ATF Jets - Booking Confirmed ${bookingRef}`,
