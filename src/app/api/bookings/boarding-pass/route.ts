...
export async function GET(req: NextRequest) {
  try {
    ...
    const boardingPassData = {
      ...
      ticketNumber: "ATF" + Date.now().toString(36).toUpperCase().slice(-8),
      ...
      // QR data
      qrData: `${process.env.NEXT_PUBLIC_APP_URL || "https://atfjets.com"}/verify?ref=${booking.bookingReference}&fn=${encodeURIComponent(flight.flightNumber)}&pax=${encodeURIComponent(passenger.lastName + "/" + passenger.firstName)}&seat=${seatNumber}&dep=${flight.departure?.airportCode}&arr=${flight.arrival?.airportCode}&date=${departureTime.toISOString().split("T")[0]}&seq=${String(paxIndex + 1).padStart(3, "0")}`,
      // Total passengers
